...
const AnimatedParticles = () => {
  ...
  useEffect(() => {
    ...
    const newParticles = Array.from({ length: particleCount }, (_, i) => ({
      ...
      color: `rgba(${Math.floor(Math.random() * 100 + 155)}, 0, 0, ${Math.random() * 0.2})`,
    }));
    ...
    <div className="min-h-screen bg-gradient-to-b from-black to-gray-900 flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="fixed top-0 left-0 w-full h-full overflow-hidden pointer-events-none">
        <div className="absolute w-full h-full">
          <div className="absolute w-[200%] h-[200%] top-[-50%] left-[-50%] bg-gradient-radial from-red-900/10 to-transparent animate-pulse-slow" 
               style={{animationDuration: '15s'}}></div>
          <div className="absolute w-[200%] h-[200%] top-[-40%] left-[-30%] bg-gradient-radial from-red-700/5 to-transparent animate-pulse-slow" 
               style={{animationDuration: '18s', animationDelay: '2s'}}></div>
          <div className="absolute w-[150%] h-[150%] top-[-25%] left-[-25%] bg-gradient-radial from-red-600/10 to-transparent animate-pulse-slow" 
      ...
      <div className="fixed top-0 left-0 w-full h-full overflow-hidden pointer-events-none">
        <div className="absolute top-1/4 left-1/4 w-64 h-64 bg-red-600/10 rounded-full blur-3xl animate-float" 
             style={{animationDuration: '15s'}}></div>
        <div className="absolute bottom-1/4 right-1/4 w-96 h-96 bg-red-900/10 rounded-full blur-3xl animate-float-reverse" 
             style={{animationDuration: '20s'}}></div>
        <div className="absolute top-1/2 right-1/3 w-48 h-48 bg-red-700/10 rounded-full blur-3xl animate-float" 
             style={{animationDuration: '18s', animationDelay: '5s'}}></div>
        <div className="absolute bottom-1/2 left-1/3 w-72 h-72 bg-red-800/10 rounded-full blur-3xl animate-float-reverse" 
      ...
      <div className="w-full max-w-5xl flex flex-col md:flex-row rounded-3xl overflow-hidden shadow-[0_0_50px_rgba(255,0,0,0.2)] bg-gradient-to-br from-gray-900 to-black relative z-10 border border-gray-800">
        ...
        <div className="md:w-2/5 p-10 relative overflow-hidden bg-[#0e0e0e]">
          <div className="absolute inset-0">
            <div className="absolute -top-20 -left-20 w-40 h-40 bg-red-600 rounded-full opacity-10 blur-3xl animate-pulse" 
                 style={{animationDuration: '8s'}}></div>
            <div className="absolute -bottom-20 -right-20 w-40 h-40 bg-red-800 rounded-full opacity-10 blur-3xl animate-pulse" 
          ...
          <div className="relative z-10 h-full flex flex-col justify-between">
            <div className="space-y-6">
              <div className="inline-block relative">
                <h1 className={`text-4xl ${uncialAntiqua.className} font-extrabold text-white`}>
                  Kolaahal<span className="text-red-500">2025</span>
                </h1>
                <div className="h-1 w-16 bg-gradient-to-r from-red-500 to-red-700 mt-2 animate-pulse" 
              ...
              <div className="pt-6 space-y-4">
                {[
                  ...
                  <div key={i} className="flex items-center space-x-3 hover:translate-x-1 transition-transform duration-300">
                    <div className="w-8 h-8 rounded-full bg-gradient-to-br from-red-500/20 to-red-700/20 flex items-center justify-center text-white animate-pulse" 
                  ...
                  {[1, 2, 3].map((i) => (
                    <div key={i} className="flex items-center space-x-3">
                      <div className="w-2 h-2 rounded-full bg-red-500 animate-pulse" 
                           style={{animationDuration: `${2 + i}s`}}></div>
                      <div className="h-0.5 flex-1 bg-gray-800 relative overflow-hidden">
                        <div className="absolute top-0 left-0 w-full h-full bg-gradient-to-r from-red-500/30 to-transparent animate-slide-right" 
                             style={{animationDuration: `${7 + i * 2}s`, animationIterationCount: 'infinite'}}></div>
                      </div>
                    </div>
                  ))}
                </div>
        ...
        <div className="md:w-3/5 p-10 relative">
          ...
          <div className="absolute top-0 left-0 w-full h-full pointer-events-none overflow-hidden">
            <div className="absolute -top-1/2 -left-1/2 w-full h-full bg-gradient-radial from-red-500/5 to-transparent rounded-full animate-move-slow" 
                 style={{animationDuration: '20s'}}></div>
          </div>
          
          <div className="relative z-10">
            <h2 className="text-3xl font-bold mb-2 flex items-center">
              <span className="bg-clip-text text-transparent bg-gradient-to-r from-red-400 to-red-600">Registration</span>
              <div className="ml-4 flex-1 h-px bg-gradient-to-r from-red-700 to-transparent relative overflow-hidden">
                <div className="absolute top-0 left-0 w-full h-full bg-gradient-to-r from-red-500/50 to-transparent animate-slide-right" 
                     style={{animationDuration: '3s', animationIterationCount: 'infinite'}}></div>
              </div>
            </h2>
            <p className="text-gray-400 mb-8">Fill in your details to complete your registration</p>
            
            <form onSubmit={handleSubmit} className="space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {[
                  { id: 'name', label: 'Full Name', type: 'text', placeholder: 'John Doe' },
                  { id: 'college', label: 'College', type: 'text', placeholder: 'Your College' },
                  { id: 'course', label: 'Course', type: 'text', placeholder: 'e.g. Computer Science' },
                  { id: 'branch', label: 'Branch', type: 'text', placeholder: 'e.g. Engineering' },
                  { id: 'contactNo', label: 'Contact No.', type: 'tel', placeholder: '+1 (123) 456-7890' }
                ].map((field) => (
                  <div key={field.id} className="space-y-1 group">
                    <label htmlFor={field.id} className={`text-sm font-medium transition-colors duration-300 ${formFocus[field.id] ? 'text-red-400' : 'text-gray-400'}`}>
                      {field.label}
                    </label>
                    <div className="relative">
                      <input
                        ...
                        required
                      />
                      <div className={`absolute bottom-0 left-0 w-full h-0.5 bg-gradient-to-r from-red-500 to-transparent transform transition-transform duration-300 ${formFocus[field.id] ? 'scale-x-100' : 'scale-x-0'}`}></div>
                    </div>
                  </div>
                ))}
                
                <div className="space-y-1 group">
                  <label htmlFor="year" className={`text-sm font-medium transition-colors duration-300 ${formFocus.year ? 'text-red-400' : 'text-gray-400'}`}>
                    Year
                  </label>
                  <div className="relative">
                    <select
                    ...
                    <div className={`absolute bottom-0 left-0 w-full h-0.5 bg-gradient-to-r from-red-500 to-transparent transform transition-transform duration-300 ${formFocus.year ? 'scale-x-100' : 'scale-x-0'}`}></div>
                  </div>
                </div>
                
                <div className="space-y-1 md:col-span-2 group">
                  <label htmlFor="email" className={`text-sm font-medium transition-colors duration-300 ${formFocus.email ? 'text-red-400' : 'text-gray-400'}`}>
                    Email ID
                  </label>
                  <div className="relative">
                    <input
                      ...
                      required
                    />
                    <div className={`absolute bottom-0 left-0 w-full h-0.5 bg-gradient-to-r from-red-500 to-transparent transform transition-transform duration-300 ${formFocus.email ? 'scale-x-100' : 'scale-x-0'}`}></div>
                  </div>
                </div>
              </div>
      ...
      <style jsx global>{`
        ...
        @keyframes particle {
          ...
          100% { transform: rotate(0) translateX(100px); opacity: 0; }
        }
